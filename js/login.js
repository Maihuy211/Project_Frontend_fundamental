document.getElementById("btn").onclick = function() {
    ...
    if (check) return;
    if (!emailValue.includes("@") || !(emailValue.endsWith(".com") || emailValue.endsWith(".net") || emailValue.endsWith(".vn") || emailValue.endsWith(".org"))) {
        errorEmail.textContent = "Email không hợp lệ!";
        email.classList.add("error-border");
        return;
    }
    let storedPass = localStorage.getItem(emailValue);
    if (!storedPass) {
        errorEmail.textContent = "Email chưa được đăng ký!";
        email.classList.add("error-border");
        return;
    }
    if (passValue === storedPass) {
